<?php
    ...
    while( $lista_productos = mysqli_fetch_assoc($consulta_productos))
    {
        $nombre_complemento = $lista_productos["nombre"];
        $producto_bloqueado = 0;
        $descripcion_complemento = $lista_productos["descripcion"];
        $post_complemento = $tipo_extra.$extra;
        $cantidad_complemento = $_POST["$post_complemento"];
        $extra++;
        ...
        switch($esquema)
        {
            case 1:
                ...
                if($lista_productos["id_producto"] == "2021")
                {
                    $producto_bloqueado = 1;
                }
                break;
            case 2:
                $precio_complemento = $lista_productos["precio_venta"];
                if($lista_productos["id_producto"] == "2124")
                {
                    $producto_bloqueado = 1;
                }
                break;
        ...
        if($cantidad_complemento != 0 && $producto_bloqueado != 1)
        {
            $control_extras++;
...
';
        ...
        if($cantidad_complemento != 0 && $producto_bloqueado != 1)
        {
            ...
            $pdf->Cell(30, 30, "$cantidad_complemento", 0,0,'C',$llenado);
            ////// CANTIDAD //////
            ...
            $pdf->MultiCell(95, 18, "$descripcion_complemento", 0, 'L', $llenado,0,'','',true,0,false,true,18,'M');
            ////// DESCRIPCIÓN (DESCRIPCIÓN) //////
            ...
            $pdf->SetX(140);
            $pdf->SetFont('helvetica','B',10);
            $pdf->MultiCell(30, 30, "$precio_complemento_txt", 0, 'C', $llenado,0,'','',true,0,false,true,30,'M');
            ////// PRECIO UNITARIO //////
            ...
            $pdf->SetX(170);
            $pdf->SetFont('helvetica','B',10);
            $pdf->MultiCell(31, 30, "$importe_complemento_txt", 0, 'C', $llenado,0,'','',true,0,false,true,30,'M');
            ////// PRECIO UNITARIO //////
            
            $altura_concepto = $altura_concepto + 30;
