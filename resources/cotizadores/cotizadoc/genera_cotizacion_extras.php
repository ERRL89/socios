<?php
    ...
    switch($producto)
    {
        case 1: // LEASING
        {
            ...
            $cantidad_producto=1;
            $clave_kit = $_POST['kit'];
            $extras = $_POST['adicional'];
            $consulta_productos = mysqli_query($con, "SELECT * FROM productos WHERE id_producto = '$clave_kit'") or die('Consulta de kit fallida -- Error: ' . mysqli_errno($con));	    
            ...
            switch($tipo_producto)
            {
                ...
                case "interfon":
                    ...
                    switch($uso_interfon)
                    {
                        ...
                        case "otro":
                            $clave_producto = 2069;
                            $cantidad_casas = $_POST['cantidad_casas'];
                            break;
                    }
                    break;
            }
        ...
        case 9: // CERRADURA ELECTRÓNICA
        {
            ...
            {
                case "hotel":
                    $nombre_producto = "Cerradura electrónica para Hotelería";
                    $descripcion_producto = "Una Cerradura electrónica para Hotelería";
                    switch($esquema)
                    {
                        case 1:
                            $precio = 666;
                    ...
                    break;
                case "casa":
                    $nombre_producto = "Cerradura electrónica doméstica";
                    $descripcion_producto = "Una Cerradura electrónica doméstica";
                    switch($esquema)
                    {
                        case 1:
                            $precio = 666;
                    ...
                    break;
                case "airbnb":
                    $nombre_producto = "Cerradura electrónica uso rudo";
                    $descripcion_producto = "Una Cerradura electrónica uso rudo";
                    switch($esquema)
                    {
                        case 1:
                            $precio = 666;
    ...
    $pdf->Cell(186, 10, "COTIZACIÓN (BASE)", 0, 0, 'R');
    ...
    $pdf->Cell(30, 80, "$cantidad_producto", 0,0,'C');
    ////// CANTIDAD //////
    ...
    $pdf->MultiCell(95, 68, "$descripcion_producto", 0, 'L', 0,0,'','',true,0,false,true,68,'M');
    ////// DESCRIPCIÓN (DESCRIPCIÓN) //////
    ...
    $pdf->SetX(140);
    $pdf->SetFont('helvetica','B',10);
    $pdf->MultiCell(30, 80, "$precio_txt", 0, 'C', 0,0,'','',true,0,false,true,80,'M');
    ////// PRECIO UNITARIO //////
    ...
    $pdf->SetX(170);
    $pdf->SetFont('helvetica','B',10);
    $pdf->MultiCell(31, 80, "$importe_producto_txt", 0, 'C', 0,0,'','',true,0,false,true,80,'M');
    ////// PRECIO UNITARIO //////

    $altura_concepto = $altura_concepto + 80;
    ...
    if($extras == "si")
    {
        ...
        // ESTABLECE LOS PRODUCTOS DERIVADOS PARA LOS KITS BÁSICOS //
        if($clave_kit == "2001" || $clave_kit == "2002" || $clave_kit == "2003")
        {
            $consulta_productos = mysqli_query($con, "SELECT * FROM productos WHERE tipo = 'extra' AND categoria = 'cctv' AND (id_producto != '2037' AND id_producto != '2021' AND id_producto != '2038')") or die('Consulta 1 fallida: ' . mysqli_error($con));	    
            $tipo_extra = "cctv_extra_";
            require "coloca_extras.php";
            
        }
        else if($clave_kit == "2004" || $clave_kit == "2005" || $clave_kit == "2008")
        {
            $consulta_productos = mysqli_query($con, "SELECT * FROM productos WHERE tipo = 'serie_a' AND categoria = 'alarma'") or die('Consulta 1 fallida: ' . mysqli_error($con));	    
            $tipo_extra = "serie_a_extra_";
            require "coloca_extras.php";
            
        }
        else if($clave_kit == "2006" || $clave_kit == "2007")
        {
            $consulta_productos = mysqli_query($con, "SELECT * FROM productos WHERE tipo = 'serie_r' AND categoria = 'alarma'") or die('Consulta 1 fallida: ' . mysqli_error($con));	    
            $tipo_extra = "serie_r_extra_";
            require "coloca_extras.php";
            
        }
        else if($clave_kit == "2009")
        {
            $consulta_productos = mysqli_query($con, "SELECT * FROM productos WHERE tipo = 'extra' AND categoria = 'cctv' AND id_producto != '2037'") or die('Consulta 1 fallida: ' . mysqli_error($con));  
            $tipo_extra = "cctv_extra_";
