...
foreach($kit_final as $componente) {
    ...
    {
        $precio_unitario = $precio_complemento;
        $precio_unitario_total = "$ ".$precio_complemento_txt;
        ...
        $pdf->SetX(15);
        $pdf->SetFont('helvetica', '', $tam_cantidad);
        $pdf->Cell(31, $tam_celda, "$cantidad_complemento_txt", 0, 0, "C", $llenado);
        ////// CANTIDAD //////
...
switch($final)
{
    case 1:
        {
            ...
            $precio_unitario_total = number_format($precio_unitario_total,2,'.',',');
            $precio_unitario_total = "$ ".$precio_unitario_total;
            $iva = $subtotal * 0.16;
            $total = $subtotal + $iva;
            break;
        }
    case 2:
        {
            $precio_unitario_total = $precio_unitario * 11;
            $precio_unitario_total = round($precio_unitario_total);
            $precio_unitario_total = number_format($precio_unitario_total,2,'.',',');
            $precio_unitario_total = "$ ".$precio_unitario_total;
...
$precio_final = ($altura_subtotal) - 98;
...
$pdf->MultiCell(30, $precio_final, "$precio_unitario_total", 0, 'C', $llenado, 0, '', '', true, 0, false, true, $precio_final, 'M');
////// PRECIO UNITARIO //////
